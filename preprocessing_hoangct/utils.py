...
def _get_words_count(x):
    length = len(str(x).split())
    return length

def _get_chars_count(x):
    s = x.split()
    x = ''.join(s)
    return len(x)

def _get_avg_wordlength(x):
    words_count = _get_words_count(x)
    if word_count > 0:
        chars_count = _get_chars_count(x)
        count = chars_count / words_count
    else:
        return 0
